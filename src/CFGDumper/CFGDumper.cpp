...
public:
    ...
    bool runOnMachineFunction(MachineFunction &MF) override {
        ...
        for (const MachineBasicBlock &MBB : MF) {
            ...
            //     // Для production-style вывода опкодов нужен доступ к TargetSubtargetInfo
            //     const TargetInstrInfo *TII = MF.getSubtarget().getInstrInfo();
            //     if (TII) {
            //          if (instrCount > 0) firstInstr = TII->getName(MBB.front().getOpcode()).str();
            //          if (instrCount > 0) lastInstr = TII->getName(MBB.back().getOpcode()).str();
            //     }
            // }
...
static RegisterPass<CFGJsonDumper> X("cfg-json-dump", "Dump CFG JSON", false, false);
