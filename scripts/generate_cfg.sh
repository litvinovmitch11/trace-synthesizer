#!/usr/bin/env bash
# Generate whole-program CFG JSON and a binary with .llvm_bb_addr_map (LTO + llc).
#
# Usage: ./scripts/generate_cfg.sh <input.cpp> [additional.cpp ...]
# Environment:
#   OUT_DIR   — artifact directory (default: output)
#   LLVM_DIR  — LLVM install prefix (default: same as Makefile / full_pipeline)

set -euo pipefail

if [[ "$#" -lt 1 ]]; then
  echo "Usage: $0 <input.cpp> [additional.cpp ...]" >&2
  exit 1
fi
...
INPUT_FILES=("$@")
BASENAME=$(basename "${INPUT_FILES[0]}" | cut -d. -f1)
...
if [[ ! -f "$PLUGIN_SO" ]]; then
  echo "Error: Plugin not found. Please run 'make build' first." >&2
  exit 1
fi
