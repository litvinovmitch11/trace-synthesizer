...
def main():
    ...
    for opt in OPT_LEVELS:
        build_dir = ROOT / f"benchmarks/local/cpp_opt_levels/build_{opt}"
        env = os.environ.copy()
        env["OPT_LEVEL"] = f"-{opt}"
        cfg, comp, ref, _ = build_artifacts(PROG, build_dir, FUNC, env=env)
        artifacts[opt] = {"cfg": cfg, "comp": comp, "ref": ref}

    print("\n=== 2. Training Models on O0 ===")
    train_opt = "O0"
    lstm_ckpt, flat_ckpt, hrl_ckpt = train_models(
        train_opt, artifacts[train_opt]["cfg"], artifacts[train_opt]["comp"]
    )
