...
ROOT = Path(".")
OUT = ROOT / "benchmarks/local/cpp_sorting_mutation/out"
OUT.mkdir(parents=True, exist_ok=True)

PROG_BASE = ROOT / "benchmarks/local/cpp_sorting_mutation/sort_base.cpp"
BUILD_BASE = ROOT / "benchmarks/local/cpp_sorting_mutation/build_base"

PROG_MUTATED = ROOT / "benchmarks/local/cpp_sorting_mutation/sort_mutated.cpp"
BUILD_MUTATED = ROOT / "benchmarks/local/cpp_sorting_mutation/build_mutated"

FUNC = "_Z10sort_arrayPii"

def run_cmd(cmd, **kwargs):
    print(f"\n[EXEC] {' '.join(cmd)}")
    subprocess.run(cmd, check=True, **kwargs)

def build_artifacts(src, build_dir, name):
    build_dir.mkdir(parents=True, exist_ok=True)
    run_cmd(["bash", "scripts/build_cpp_dataset_artifacts.sh", str(src), str(build_dir)])

    cfg = build_dir / f"{name}.cfg.json"
    comp = build_dir / f"{name}.compressed_trace.json"
    ...
    ref_path = build_dir / f"{name}_reference_intra.json"
...
def main():
    ...
    cfg_base, comp_base, ref_base = build_artifacts(PROG_BASE, BUILD_BASE, "sort_base")
    cfg_mut, comp_mut, ref_mut = build_artifacts(PROG_MUTATED, BUILD_MUTATED, "sort_mutated")

    print("\n=== 2. Rendering Ground Truth Traces ===")
    run_cmd(["make", "visualize-trace", f"CFG={cfg_base}", f"FUNC={FUNC}", f"TRACE={ref_base}", f"OUT={OUT}/viz_true_base"])
    run_cmd(["make", "visualize-trace", f"CFG={cfg_mut}", f"FUNC={FUNC}", f"TRACE={ref_mut}", f"OUT={OUT}/viz_true_mutated"])

    print("\n=== 3. Training Synthesizers on Base Program ===")
    tb_dir = OUT / "tensorboard"
    
    # LSTM
    ds_base = create_lstm_dataset(cfg_base, comp_base, "sort_base")
    ...
    rollout_and_eval("pgo", [
        ...
        "--cfg", str(cfg_mut), "--func", FUNC, "--episodes", "10",
