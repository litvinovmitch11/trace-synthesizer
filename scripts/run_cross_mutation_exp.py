...
ROOT = Path(".")
OUT = ROOT / "benchmarks/local/cpp_mutation/out"
OUT.mkdir(parents=True, exist_ok=True)

PROG_BASE = ROOT / "benchmarks/local/cpp_mutation/trigger_base.cpp"
BUILD_BASE = ROOT / "benchmarks/local/cpp_mutation/build_base"

PROG_MUTATED = ROOT / "benchmarks/local/cpp_mutation/trigger_mutated.cpp"
BUILD_MUTATED = ROOT / "benchmarks/local/cpp_mutation/build_mutated"
...
def main():
    ...
    cfg_base, comp_base, ref_base = build_artifacts(PROG_BASE, BUILD_BASE, "trigger_base")
    cfg_mut, comp_mut, ref_mut = build_artifacts(PROG_MUTATED, BUILD_MUTATED, "trigger_mutated")

    print("\n=== 2. Rendering Ground Truth Traces ===")
    run_cmd(["make", "visualize-trace", f"CFG={cfg_base}", "FUNC=main", f"TRACE={ref_base}", f"OUT={OUT}/viz_true_base"])
    run_cmd(["make", "visualize-trace", f"CFG={cfg_mut}", "FUNC=main", f"TRACE={ref_mut}", f"OUT={OUT}/viz_true_mutated"])

    print("\n=== 3. Training Synthesizers on Base Program ===")
    tb_dir = OUT / "tensorboard"
    
    # LSTM
    ds_base = create_lstm_dataset(cfg_base, comp_base, "trigger_base")
    ...
    rollout_and_eval("pgo", [
        ...
        "--cfg", str(cfg_mut), "--func", "main", "--episodes", "10",
