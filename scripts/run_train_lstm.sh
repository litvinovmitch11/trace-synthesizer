...
CMD=(poetry run python3 scripts/train_feature_window_lstm.py \
  --dataset-jsonl "$DATASET" \
  --out-stem "$OUT_DIR/model" \
  --window-back 8 \
  --epochs "$EPOCHS" \
  --seed 42 \
  --train-report "$OUT_DIR/report.json")

if [[ -n "$TB_LOGDIR" ]]; then
  CMD+=(--tb-logdir "$TB_LOGDIR" --tb-run-name "train_lstm")
fi
